...
function obtenerEfecto(nombre: EFECTO): string {
  if (nombre === EFECTO.ACIERTO) return acierto;
  else if (nombre === EFECTO.ERROR) return error;
  else if (nombre === EFECTO.INGRESAR) return ingresar;
  else if (nombre === EFECTO.JUGAR_DE_NUEVO) return jugarDeNuevo;
  else if (nombre === EFECTO.SIGUIENTE_NIVEL) return siguienteNivel;
  else if (nombre === EFECTO.MENU) return menu;
  else return comenzarJuego;
}
