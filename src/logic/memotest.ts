...
export default class Memotest{
    ...
    #constructorTablero: ConstructorTablero = new ConstructorTablero(config.colores);
    ...
    #mismoColor(cuadradoElegido: EstadoCuadrado, ultimoCuadradoElegido: EstadoCuadrado): boolean {
        return (cuadradoElegido.color === ultimoCuadradoElegido.color);
    }
    
    #manipularCuadrado(id: number, cuadradoElegido: EstadoCuadrado, acertado: boolean): void {
    ...
    eleccion(id: number): boolean | undefined {
        ...
        const ultimoCuadradoElegido: EstadoCuadrado | undefined = this.#tablero.get(idUltimoElegido);//Obtenemos valores nombre y elegido
        if (!ultimoCuadradoElegido) return false;
        if (cuadradoElegido.elegido || !this.#mismoColor(cuadradoElegido, ultimoCuadradoElegido)) {
        ...
        this.#manipularCuadrado(id, cuadradoElegido, true)
        if (this.informacion.gano)
            this.#configurarScore();
        return true;
    }
    ...
    #configurarScore(): void {
        this.#puntajeNivel += this.#cronoNivel.info.tiempo * config.masScorePorSegundos;
    ...
    get informacion(): InformacionMemotest {
        const info: InformacionMemotest = {
            ...
            ultimoNivelTerminado: (this.#tablero.size === this.#acertados.length) && 
