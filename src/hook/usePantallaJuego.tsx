...
function usePantallaJuego(informacion: Informacion): ControladorPantallasJuego {
    const [ pantalla , setPantalla ] = useState<PANTALLA_JUEGO>(PANTALLA_JUEGO.JUEGO);
    ...
    const cambiarPantalla = (): void => {
        if (final)
            return setPantalla(PANTALLA_JUEGO.FINAL);
        else if (gano)
            return setPantalla(PANTALLA_JUEGO.VICTORIA);
        else if (juego)
            return setPantalla(PANTALLA_JUEGO.JUEGO);
        else 
            return setPantalla(PANTALLA_JUEGO.DERROTA)
    }
