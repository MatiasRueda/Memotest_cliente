...
const encontrarPar = (id: number, estado: EstadoCuadrado , cuadrados: Map<number, EstadoCuadrado>): number => {
    let idEncontrado: number = -1;
    cuadrados.forEach((estadosVerificar, idVerificar ) => {
        if (idVerificar === id || idEncontrado !== -1 || estadosVerificar.color !== estado.color)
            return ;
...
const encontrarPares = (cuadrados: Map<number, EstadoCuadrado>):  Map<number, number> => {
    let parEncontrado: Map<number, number>  = new Map();
    let idsYaEncontrados: number[] = [];
    cuadrados.forEach((estado, id) =>  {
        if (idsYaEncontrados.find((idEncontrado) => idEncontrado === id))
            return;
...
describe("Parte 2 - Valores iniciales correctos", () => {
    ...
    test("Se puede cambiar el estado de todos los cuadrados", () => {
        ...
        const cuadradosPostCambio: Map<number, EstadoCuadrado> = memotest.informacion.tablero;
        cuadradosPostCambio.forEach((estado, id) => estadoPostCambio &&= estado.elegido);
